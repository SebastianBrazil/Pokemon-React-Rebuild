...
const PopupComponent = (props: checkIfOpen) => {
    ...
    const [recallPoke, setRecallPoke] = useState<string>();
        ...
        <>
            {
                (sentence !== "Available" || list) && < div className="absolute 2xl:movePls1 sm:movePls2 movePls3 lg:movePls4" >
                    <div className="hidee ml-6 sm:ml-8 mx-2 favoritesColor w-56 sm:w-80 rounded-2xl sm:rounded-3xl border border-black">
                        {
                            list && list.map((pokeName, key) => {
                                return <div className="flex justify-start ml-5 mt-3 mb-3">
                                    <p key={key+1} className="text-left sm:text-2xl kotta">{pokeName}</p>
                                    <img key={key+1} className="w-8 h-8 ml-3" src={heart} alt="Favorite Heart Inside Favorite List" />
                                </div>
                            })
                        }
